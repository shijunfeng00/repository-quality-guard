...
### 新增 QG 时，也把“希望怎么修”写进 RULES

Profile 解决的是**这个项目要把某条规则管得多严**；`references/RULES.md` 还应该说明**这条规则到底在防什么，以及正确修复通常应该往哪里走**。因此以后给 RQG Core 增加新的 `QGxxx`，不要只补检测器和严重度；如果修复方向不是显而易见的，建议在 `RULES.md` 同时写清楚：

- **问题是什么**：命中背后的真实工程风险，而不只是语法现象；
- **希望怎么改**：通常应该先追哪个 owner、producer、schema、caller 或边界；
- **不希望怎么改**：哪些 helper、fallback、兼容层或同义语法只是把问题换皮；
- **关闭证据**：什么上下游事实和验证足以说明根因真的被处理。

不要求给一两百条 QG 每条写一篇长文。相近规则可以共用一张“修复卡”；目标只是避免 Coding Agent 把“让 QG 编号消失”误当成“问题已经修好”。

项目独有的约束仍然放在 Profile；只有足够通用、准备进入 RQG Core 的规则，才需要把通用修复意图同步沉淀到 `RULES.md`，让其他开发者不依赖私有项目背景也能理解。
