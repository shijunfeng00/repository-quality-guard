...
from runtime.src.project_profiles import resolve_profile_reference
...
def _write_profile(root: Path, name: str) -> None:
    ...
    (p / "profile.json").write_text(
        json.dumps({"schema": "repository-quality-guard/profile-v1", "name": name, "entrypoint": "extension.py"}),
        encoding="utf-8",
    )
...
class TestProfileResolution(unittest.TestCase):
    def test_portable_explicit_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual((selected.name, selected.source), ("alpha", "explicit-cli"))
    ...
    def test_portable_exact_directory_name_auto_match(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual((selected.name, selected.source), ("alpha", "auto-directory-name"))
    ...
    def test_portable_does_not_guess_from_marker_files(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            (repo / "agents" / "react_agent.py").write_text("# marker\n", encoding="utf-8")
            _write_lock(release, "skill")
            _write_profile(release, "alpha")
            selected = resolve_profile_reference(repo, release_root=release)
    ...
    def test_installed_profile_comes_only_from_sealed_payload(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            (installed / "POLICY.json").write_text(
                json.dumps({"schema": "repository-quality-guard/installed-policy-v1", "profile_name": "alpha", "profile_version": "1", "profile_source": "explicit-cli"}),
                encoding="utf-8",
            )
            (profile / "profile.json").write_text(
                json.dumps({"schema": "repository-quality-guard/profile-v1", "name": "alpha", "entrypoint": "extension.py"}),
                encoding="utf-8",
            )
            ...
            selected = resolve_profile_reference(repo, release_root=release)
            self.assertEqual((selected.name, selected.source), ("alpha", "sealed-installed"))
    ...
    def test_installed_generic_is_frozen(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            (installed / "POLICY.json").write_text(
                json.dumps({"schema": "repository-quality-guard/installed-policy-v1", "profile_name": "", "profile_version": "", "profile_source": "generic"}),
                encoding="utf-8",
            )
            selected = resolve_profile_reference(repo, release_root=release)
            self.assertEqual((selected.name, selected.source), ("", "sealed-installed-generic"))
