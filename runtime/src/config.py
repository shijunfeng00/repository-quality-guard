...
def is_tool_generated_path(path: str | Path) -> bool:
    """Return whether ``path`` is a disposable artifact generated by RQG.

    Args:
        path: Repository-relative path to classify.

    Returns:
        ``True`` when the path belongs to RQG's regenerable derived output.
    """
    normalized = Path(path).as_posix().lstrip("./")
    return any(fnmatch(normalized, pattern) for pattern in _GENERATED_TOOL_PATTERNS)
